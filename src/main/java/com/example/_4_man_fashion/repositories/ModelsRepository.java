...
public interface ModelsRepository extends JpaRepository<Models, Integer> {
    ...
    @Query("select model from Models model where model.status = 1 and model.models_name = :name order by model.id")
    List<Models> findByName(@Param("name") String name);

    @Query("select model from Models model where model.status = 0 and model.models_name = :name order by model.id")
